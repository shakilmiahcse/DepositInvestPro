		    ...
		    <div class="card-header d-flex align-items-center">
				@if(isset($_GET['type']))
					<span class="panel-title">
						{{ $_GET['type'] == 'deposit_requests' ? _lang('Deposit Requests') : ($_GET['type'] == 'withdraw_requests' ? _lang('Withdraw Requests') : _lang('Transfer Requests')) }}
					</span>
                @else
				...
				<select name="type" id="type" class="ml-auto auto-select filter-select" data-selected="{{ isset($_GET['type']) ? $_GET['type'] : 'deposit_requests' }}">
					...
					<option value="transfer_requests">{{ _lang('Transfer Request') }}</option>
				</select>
			</div>
			<div class="card-body">
				<table class="table table-bordered data-table">
					...
					<tbody>
						@php $request_type = isset($_GET['type']) ? $_GET['type'] : 'deposit_requests'; @endphp

						@if($request_type == 'transfer_requests')
							@foreach($transfer_requests as $transaction_request)
								<tr>
									...
									<td>{{ $transaction_request->method }}</td>
									<td>{!! xss_clean(transaction_status($transaction_request->status)) !!}</td>
								</tr>
							@endforeach
						@else
							@php $transaction_requests = $request_type == 'deposit_requests' ? $deposit_requests : $withdraw_requests; @endphp
